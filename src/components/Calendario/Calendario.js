...
const Calendar = () => {
    ...
    const renderCalendar = () => {
        ...
        for (let i = 0; i < calendar.length; i++) {
            ...
            cells.push(
                <Day key={ i } className={ calendar[i] === currentDate.dia && month === new Date().getMonth() && year === new Date().getFullYear() ? 'active' : '' }>
                    { calendar[i] }
                    { calendar[i] === currentDate.dia && month === new Date().getMonth() && year === new Date().getFullYear() && (
                        <><br />{ currentDate.hora }:{ currentDate.min } { currentDate.texto }</>
                    ) }
                </Day>
            );
            dia++;
    ...
    return (
        <div>
            ...
            <CalendarTable>
                <CalendarHeader>
                    <tr>
                        <td colSpan="7">{ monthName } { year }</td>
                    </tr>
                    <tr>
                        <td>Lunes</td>
                        <td>Martes</td>
